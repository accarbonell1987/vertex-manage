...
interface Props {
  ...
  onClose: () => void;
  onConfirm: (cleanAllReferrals: boolean, applyPenaltiesToAll: boolean, penalizedToAll: boolean) => void;
}
...
const StreamerManagerPanel = ({ open, setOpen, onClose, onConfirm }: Props) => {
  ...
  const [penalizedToAll, setPenalizedToAll] = useState<boolean>(false);
        ...
        <form className="space-y-4">
          <Card>
            <CardContent className="flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                <Switch id="applyPenalties" checked={penalizedToAll} onCheckedChange={setPenalizedToAll} />
                <Badge className={`flex items-center gap-1 ${penalizedToAll ? 'bg-green-600' : 'bg-red-600'}`}>
                  {penalizedToAll ? 'SÍ' : 'NO'}
                </Badge>
                <Label htmlFor="applyPenalties">Todos penalizados </Label>
              </div>
            </CardContent>
          </Card>
          <div className="flex justify-end gap-2 w-full">
            ...
            <Button
              ...
              onClick={() => onConfirm(cleanAllReferrals, applyPenaltiesToAll, penalizedToAll)}
            >
              Aplicar
            </Button>
          </div>
