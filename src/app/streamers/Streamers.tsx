...
const Streamers = ({ init }: Readonly<{ init: StreamerWithReferals[] }>) => {
  ...
  const handleOnConfirmApplyConfiguration = (cleanAllReferrals: boolean, applyPenaltiesToAll: boolean, penalizedToAll: boolean) => {
    if (cleanAllReferrals) {
      deleteAllReferrals();
    }
    updateStreamerPenaltiesToAll({ applyPenaltiesToAll, penalizedToAll });
    handleOnClose();
  };
